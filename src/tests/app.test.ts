...
import { initApp, updateGlobalMoneyUI } from '../app';
...
describe('Application Entry Point', () => {
  ...
  it('should initialize app and return a valid state', async () => {
    ...
    vi.spyOn(window, 'fetch').mockImplementation(async (url: string) => {
      ...
      if (url.includes('git-info.txt')) {
        return Promise.resolve({ ok: true, text: () => Promise.resolve('abc123def\nInitial commit') });
      }
  ...
  it('should load app info from public/MCC/', async () => {
    ...
    vi.spyOn(window, 'fetch').mockImplementation(async (url: string) => {
      if (url.includes('version.txt')) {
        return Promise.resolve({ ok: true, text: () => Promise.resolve('2.0.0') });
      }
      if (url.includes('git-info.txt')) {
        return Promise.resolve({ ok: true, text: () => Promise.resolve('def456ghi\nBetter button') });
      }
      return Promise.reject(new Error('Not found'));
    });
  ...
  it('should handle negative money correctly', () => {
    const mockAppState: AppState = {
      currentTab: 'mines',
      devMode: false,
      version: null,
    ...
    const globalMoney = document.getElementById('global-money');
    expect(globalMoney?.textContent).toBe('$-100');
  });
});
