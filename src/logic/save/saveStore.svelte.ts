...
export function createSaveStore() {
  ...
  return {
    ...
    buildPersistedGameState(gameState: GameState, navigation?: Partial<SavedNavigation>): PersistedGameState {
      return {
    ...
    saveToLocalStorage(
      ...
      try {
        ...
        state.lastError = error instanceof Error ? error.message : 'Failed to save to localStorage.';
        return false;
      }
    },
    ...
    loadFromLocalStorage(): PersistedGameState | null {
      ...
      try {
        ...
        state.lastError = error instanceof Error ? error.message : 'Failed to load from localStorage.';
        return null;
      }
    },
    ...
    clearLocalStorageSave(): boolean {
      ...
      try {
        ...
        state.lastError = error instanceof Error ? error.message : 'Failed to clear local save.';
        return false;
      }
    },
    ...
    downloadSaveFile(
      ...
      try {
        ...
        state.lastError = error instanceof Error ? error.message : 'Failed to download save file.';
        return false;
      }
    },
  };
