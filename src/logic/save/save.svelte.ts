...
import { backfillFactoryResources } from '../world/worldGen';
...
function applyLoadedState(snapshot: PersistedGameState): void {
  ...
  // Factories generated by ring generation never stored what they buy — the ore
  // was drawn to theme the name and then discarded. The name still encodes it,
  // so repair on load rather than making the player start over.
  const repairedFactories = backfillFactoryResources(snapshot.world);
  if (repairedFactories > 0) {
    log.info('load', `Recovered accepted resources for ${repairedFactories} factories from their names.`);
  }
