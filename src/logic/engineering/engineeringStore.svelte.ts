...
export function createEngineeringStore(initial?: Partial<EngineeringState>) {
  ...
  return {
    ...
    spendEngineeringIdeas(amount = 1): boolean {
      ...
      if (state.engineeringIdeas < amount) {
        return false;
      }
    ...
    unlockUndergroundLevel(amount = 1) {
      state.maxUndergroundLevels = clampMin(state.maxUndergroundLevels + amount, 0);
    },
    ...
    applyResetReward(engineeringIdeasGained: number) {
      state.resetCount += 1;
      state.engineeringIdeas = clampMin(state.engineeringIdeas + engineeringIdeasGained, 0);
    },
  };
}
