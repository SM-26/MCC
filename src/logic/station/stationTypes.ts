...
export function hasPlatformAtDepth(
    station: Station,
    northExpansionIndex: number,
    depth: number,
): boolean {
    return station.platforms.some(
        (platform) =>
            platform.northExpansionIndex === northExpansionIndex &&
            platform.depth === depth,
    );
}

export function getPlatformsForNorthExpansion(
    station: Station,
    northExpansionIndex: number,
): Platform[] {
    return station.platforms
        .filter((platform) => platform.northExpansionIndex === northExpansionIndex)
        .sort((a, b) => a.depth - b.depth);
}

export function getPlatformDisplayIndex(
    station: Station,
    platform: Platform,
): number {
    const ordered = getPlatformsForNorthExpansion(
        station,
        platform.northExpansionIndex,
    );
