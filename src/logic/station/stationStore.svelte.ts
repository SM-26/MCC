    ...
    const state = $state<Station>(
        initial
            ? {
                ...
                trainyardInventory:
                    initial.trainyardInventory ?? createEmptyTrainyardInventory(),
            }
    ...
    return {
        ...
        addPlatform(northExpansionIndex: number, depth: number): boolean {
            ...
            state.platforms.push(
                createPlatform(
                    createPlatformId(northExpansionIndex, depth),
                    northExpansionIndex,
                    depth,
                ),
            );
        ...
        assignTrainToPlatform(platformId: PlatformId, engineAge: Ages): boolean {
            ...
            if (platform.train) return false;
        ...
        updateTrainCart(
            platformId: PlatformId,
            cartIndex: number,
            updates: Partial<CartSlot>,
        ): boolean {
            const platform = this.getPlatform(platformId);
            if (!platform?.train) return false;
            if (cartIndex < 0 || cartIndex >= platform.train.carts.length) return false;
